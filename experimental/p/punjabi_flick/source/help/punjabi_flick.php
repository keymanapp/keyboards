...
?>
  ...
  <p>Punjabi Flick is based on the clustering of syllables according to the area of the mouth used that is inherent to
    Gurmukhi and many other Indian scripts. Thus, we get a character set
  ...
  <p>Each <em>akshara</em> (syllable) key is equipped with the ability to produce not only the character on the key, but
    ...
    direction of the desired character. For example, flicking the ਕ key to the right will produce ਖ, flicking it up will
    produce ਗ, and so on. The characters available on flicking are displayed on the key itself. They are ordered in
  ...
  <p>If you notice, the only vowels available on the key are the short vowels ਅ, ਇ, ਉ, ਏ, ਓ. This is to utilise the fact that each of these vowels has a corresponding long
    vowel, and thus there is no need to clutter the key with these. Instead, we provide the key <strong>ਅ↔ਆ</strong>
    which converts short vowels to long vowels and vice versa. This also works on their matras (diacritics).</p>
  <p>For example, ਕ + ਇ = ਕਿ, and pressing <strong>ਅ↔ਆ</strong> makes it की.</p>
  <p>Speaking of diacritics, all the vowels have the corresponding diacritics as their output when a consonant is found
    as the previous character (except ਅ, which outputs the ਾ diacritic, as each consonant has the ਅ sound by default and
    ਾ is probably one of the most frequently used matras). The keyboard has rules in place to dictate what should happen
    in different scenarios. A few of these include:</p>

  <ul>
    <li>If a <em>consonant</em> is found before adding a vowel, the vowel's diacritic is added. For example, ਕ + ਇ = ਕਿ.
    </li>
    <li>If a <em>vowel</em> is found before adding a vowel, the whole vowel is placed, no diacritic (as it is
      nonsensical to do so). For example, ਇ + ਉ = ਇਉ, not ਇੁ.</li>
    <li>If a <em>vowel diacritic</em> is found before adding a vowel, the whole new vowel is placed. For example, ਕਿ + ਉ
      = ਕਿਉ.</li>
    <li>If an <em>anusvara</em> (ਂ), <em>adak bindi</em> (ਁ) or <em>visarga</em> (ਃ) is found before adding a vowel,
      the whole new vowel is placed. For example, ਕਂ + ਉ = ਕਂਉ.</li>
  </ul>

  <h3>Matra to Svara Key (ੇ↔ਏ)</h3>
  <p>Sometimes, it is not desired to put a matra right after a consonant, but rather a full vowel. This is where the
    <strong>ੇ↔ਏ</strong> key comes in. It converts the matra to a full vowel and vice versa. For example, ਕ +  ੇ = ਕੇ,
    and pressing <strong>ੇ↔ਏ</strong> makes it ਕਏ.</p>
  <p>An example of a word I can imagine this would be real helpful for is ਕਈ (many), where the ਈ is a full vowel and not
    a matra.</p>
  <h3>Halant (੍)</h3>
  <p>The halant is a special character in Gurmukhi that is used to remove the inherent vowel sound from a consonant.
    It is used to create conjuncts, and is also used in many other scenarios. The halant key is placed at the bottom of
    the keyboard. Similar to the 12-kana keyboard, it also contains the flick-keys for parentheses, the Gurmukhi
    abbreviation sign, and a dash.</p>
  <h3>Bindus and nuqtas</h3>
  <p>The tenth key in the keyboard contains the bindu (ਂ), adak bindi (ਁ), visarga (ਃ), tippi (ੰ), addak (ੱ) and nukta (਼)
