<?php 
  $pagename = 'Hungarian Basic Keyboard Help';
  $pagetitle = $pagename;
  // Header we will tidy up later  
  require_once('header.php');
?>


<p>
    This keyboard layout is designed for Hungarian.  It includes an on screen keyboard which can be viewed by clicking on the Keyman icon and selecting the On Screen Keyboard menu item. The keyboard layout follows the Windows 10 Hungarian layout.
</p>

<p>This keyboard makes use of "deadkeys" which are listed below:</p>

<p>ˇc will produce č (available on cCdDeEnNrRsStTzZ).</p>
<p>^a will produce â (available on aAiIoO).</p>
<p>˘a will produce ă (available on aA).</p>
<p>°u will produce ů (available on uU).</p>
<p>˛a will produce ą (available on aAeE).</p>
<p>˙z will produce ż (available on zZ).</p>
<p>´a will produce á (available on aAcCeEiIlLnNoOrRsSuUyYzZ).</p>
<p>¸c will produce ç (available on cCsStT).</p>
<p>¨a will produce ä (available on aAeEoOuU).</p>
<p>˝o will produce ő (available on oOuU).</p>


<h1>Keyboard Layout</h1>

<h2>Desktop Keyboard Layout</h2>
<div id='osk' data-states='default shift rightalt'>
</div>

<h2>Mobile/Tablet Keyboard Layout</h2>

<div id='osk-tablet' data-states='default shift rightalt'>
</div>
