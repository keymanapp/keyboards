<?php 
  $pagename = "Dega Keyboard Help";
  $pagetitle = $pagename;
  require_once('header.php');
?>

<p>
    Dega keyboard is created for indigenous people in the Central Highlands of Vietnam, to preserve their writing, this Dega keyboard can be used by Êđê, Jơrai, Kơ’Ho, Mơnông, Bahnar and many other indigenous ethnic groups.
</p>

<p>For how to type certain characters other than the usual Latin characters, please refer to the following bullet points.
</p>

<h2>Upper Case</h2>
<ul>
<li><kbd> E </kbd> + <kbd> E </kbd> = Ê </li>
<li><kbd> E </kbd> + <kbd> X </kbd> = Ĕ </li>
<li><kbd> E </kbd> + <kbd> E </kbd> + <kbd> X </kbd> = Ê̆ </br></li>
<li><kbd> E </kbd> + <kbd> ` </kbd> = È </li>
<li><kbd> E </kbd> + <kbd> E </kbd> + <kbd> ` </kbd>= Ề </li>
</ul>

<ul>
<li><kbd> U </kbd> + <kbd> U </kbd> = Ư </li>
<li><kbd> U </kbd> + <kbd> X </kbd> = Ŭ </li>
<li><kbd> U </kbd> + <kbd> U </kbd> + <kbd> X </kbd> = Ư̆ </li>
<li><kbd> U </kbd> + <kbd> W </kbd> = Ư </li>
<li><kbd> U </kbd> + <kbd> W </kbd> + <kbd> X </kbd> = Ư̆ </li>
<li><kbd> U </kbd> + <kbd> ` </kbd> = Ù </li>
<li><kbd> U </kbd> + <kbd> U </kbd> + <kbd> ` </kbd>= Ừ </li>
<li><kbd> U </kbd> + <kbd> ^ </kbd> = Û </li>
<li><kbd> U </kbd> + <kbd> ^ </kbd> = <kbd>Û</kbd> + <kbd> X </kbd> + <kbd> X </kbd> = Û̆ </li>
</ul>

<ul>
<li><kbd> I </kbd> + <kbd> X </kbd> = Ĭ </li>
<li><kbd> I </kbd> + <kbd> ^ </kbd> = Î </li>
<li><kbd> I </kbd> + <kbd> ^ </kbd> = <kbd>Î</kbd> + <kbd> x </kbd> + <kbd> x </kbd> = Î̆ </li>
<li><kbd> Y </kbd> + <kbd> X </kbd> = Y̆ </li>
</ul>

<ul>
<li><kbd> O </kbd> + <kbd> O </kbd> = Ô </li>
<li><kbd> O </kbd> + <kbd> X </kbd> = Ŏ </li>
<li><kbd> O </kbd> + <kbd> O </kbd> + <kbd> X </kbd> = Ô̆ </li>
<li><kbd> O </kbd> + <kbd> ` </kbd> = Ò </li>
<li><kbd> O </kbd> + <kbd> O </kbd> + <kbd> ` </kbd>= Ồ </li>
</ul>

<ul>
<li><kbd> O </kbd> + <kbd> W </kbd> = Ơ </li>
<li><kbd> O </kbd> + <kbd> W </kbd> + <kbd> X </kbd> = Ơ̆ </li>
<li><kbd> O </kbd> + <kbd> W </kbd> + <kbd> ` </kbd>= Ờ </li>
</ul>

<ul>
<li><kbd> A </kbd> + <kbd> A </kbd> = Â </li>
<li><kbd> A </kbd> + <kbd> X </kbd> = Ă </li>
<li><kbd> A </kbd> + <kbd> A </kbd> + <kbd> X </kbd> = Â̆ </li>
<li><kbd> A </kbd> + <kbd> ` </kbd> = À </li>
<li><kbd> A </kbd> + <kbd> A </kbd> + <kbd> ` </kbd>= Ầ </li>
</ul>

<ul>
<li><kbd> C </kbd> + <kbd> C </kbd> =  Č </li>
<li><kbd> B </kbd> + <kbd> B </kbd> =  Ƀ </li>
<li><kbd> N </kbd> + <kbd> N </kbd> = Ñ </li>
<li><kbd> D </kbd> + <kbd> D </kbd> = Đ </li>
</ul>

<p><b>Note:</b> You can choose to release the Shift key after pressing the first letter with it to get a capital letter, just like when you press and hold it.</p>

<h2>Lower Case</h2>
<ul>
<li><kbd> e </kbd> + <kbd> e </kbd> = ê </li>
<li><kbd> e </kbd> + <kbd> x </kbd> = ĕ </li>
<li><kbd> e </kbd> + <kbd> e </kbd> + <kbd> x </kbd> = ê̆ </li>
<li><kbd> e </kbd> + <kbd> ` </kbd> = è </li>
<li><kbd> e </kbd> + <kbd> e </kbd> + <kbd> ` </kbd>= ề </li>
</ul>

<ul>
<li><kbd> u </kbd> + <kbd> u </kbd> = ư </li>
<li><kbd> u </kbd> + <kbd> x </kbd> = ŭ </li>
<li><kbd> u </kbd> + <kbd> u </kbd> + <kbd> x </kbd> = ư̆ </li>
<li><kbd> u </kbd> + <kbd> w </kbd> = ư </li>
<li><kbd> u </kbd> + <kbd> w </kbd> + <kbd> x </kbd> = ư̆ </li>
<li><kbd> u </kbd> + <kbd> ` </kbd> = ù </li>
<li><kbd> u </kbd> + <kbd> u </kbd> + <kbd> ` </kbd>= ừ </li>
<li><kbd> u </kbd> + <kbd> ^ </kbd> = û </li>
<li><kbd> u </kbd> + <kbd> ^ </kbd> = <kbd>û</kbd> + <kbd> x </kbd> + <kbd> x </kbd> = û̆ </li>

</ul>

<ul>
<li><kbd> i </kbd> + <kbd> x </kbd> = ĭ </li>
<li><kbd> i </kbd> + <kbd> ^ </kbd> = î </li>
<li><kbd> i </kbd> + <kbd> ^ </kbd> = <kbd>î</kbd> + <kbd> x </kbd> + <kbd> x </kbd> = î̆ </li>
<li><kbd> y </kbd> + <kbd> x </kbd> = y̆ </li>
</ul>

<ul>
<li><kbd> o </kbd> + <kbd> o </kbd> = ô </li>
<li><kbd> o </kbd> + <kbd> x </kbd> = ŏ </li>
<li><kbd> o </kbd> + <kbd> o </kbd> + <kbd> x </kbd> = ô̆ </li>
<li><kbd> o </kbd> + <kbd> ` </kbd> = ò </li>
<li><kbd> o </kbd> + <kbd> o </kbd> + <kbd> ` </kbd>= ồ </li>
</ul>

<ul>
<li><kbd> o </kbd> + <kbd> w </kbd> = ơ </li>
<li><kbd> o </kbd> + <kbd> w </kbd> + <kbd> x </kbd> = ơ̆ </li>
<li><kbd> o </kbd> + <kbd> w </kbd> + <kbd> ` </kbd>= ờ </li>
</ul>

<ul>
<li><kbd> a </kbd> + <kbd> a </kbd> = â </li>
<li><kbd> a </kbd> + <kbd> x </kbd> = ă </li>
<li><kbd> a </kbd> + <kbd> a </kbd> + <kbd> x </kbd> = â̆ </li>
<li><kbd> a </kbd> + <kbd> ` </kbd> = à </li>
<li><kbd> a </kbd> + <kbd> a </kbd> + <kbd> ` </kbd>= ầ </li>
</ul>

<ul>
<li><kbd> c </kbd> + <kbd> c </kbd> = č </li>
<li><kbd> b </kbd> + <kbd> b </kbd> = ƀ </li>
<li><kbd> n </kbd> + <kbd> n </kbd> = ñ </li>
<li><kbd> d </kbd> + <kbd> d </kbd> = đ </li>
</ul>

  <h2>Desktop Keyboard Layout</h2>
   <div id='osk' data-states='default shift'></div>

  <h2>Phone Keyboard Layout</h2>
   <div id='osk-phone' data-states='default shift numeric more'></div>
	
  <h2>Tablet Keyboard Layout</h2>
   <div id='osk-tablet' data-states='default shift'></div>
